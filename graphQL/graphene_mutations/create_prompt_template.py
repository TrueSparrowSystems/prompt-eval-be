...
class CreatePromptTemplateMutation(MutateBase):
    ...
    def self_mutate(cls, root, info, prompt_template_data=None):
        if not CommonValidator.max_length_validation(prompt_template_data.name, 70):
        ...
        if prompt_template_data.description:
            if not CommonValidator.max_length_validation(prompt_template_data.description, 240):
